...
public class DocumentSaver implements Saver {
    private static final String docType = ".xlsx";
    ...
    public int saveGroupToFile(List<RailroadDocument> documents, File file, RDocEnum... columnOrder) {
        ...
        try (OutputStream fileOutStream = new FileOutputStream(file)) {
            XSSFWorkbook  workbook = getWorkbookFromRailDocsList(documents, columnOrder);
    ...
    private void saveToFolder(RailroadDocument document, File folder, RDocEnum... columnOrder) throws IOException {
        ...
        try (OutputStream fileOutStream = new FileOutputStream(file)) {
            XSSFWorkbook workbook = getWorkbookFromRailDoc(document, columnOrder);
            workbook.write(fileOutStream);
        }
     ...
     */
    private XSSFWorkbook  getWorkbookFromRailDoc(RailroadDocument railroadDocument, RDocEnum... columnOrder) {
     ...
     */
    private XSSFWorkbook getWorkbookFromRailDocsList(List<RailroadDocument> documentList, RDocEnum... columnOrder) {
     ...
     */
    private void setSpecifiedValueToCell(Cell cell, RailroadDocument railDoc, RDocEnum column, int vagNumb, XSSFWorkbook workbook) {
        switch (column) {
            ...
            case PAYER_CODE:
                ...
                break;
            case TARIF_DISTANCE:
                cell.setCellValue(railDoc.getTarifDistance());
                break;
            case VAGON_NUMBER:
            ...
            case PAYMENT_SUMM:
                ...
                break;
            case VAGON_ZPU:
                cell.setCellValue(railDoc.getVagonList().get(vagNumb).getZpuList());
                break;
            case VAGON_ZPU_COUNT:
                cell.setCellValue(railDoc.getVagonList().get(vagNumb).getZpuCount());
                break;
            case VAGON_PAYMENT:
                cell.setCellValue(railDoc.getVagonList().get(vagNumb).getPayment());
                break;
            case COLUMN_7_INFO:
